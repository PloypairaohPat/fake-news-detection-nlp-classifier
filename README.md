...
- Built a fake news detection classifier on 72,134 labelled articles (WELFake dataset), achieving 97% macro F1 on a stratified held-out test set using a TF-IDF + XGBoost pipeline with a custom spaCy lemmatisation preprocessor.
- Engineered a reusable sklearn-compatible `TextPreprocessor` class combining URL/HTML stripping, spaCy lemmatisation, and stopword removal, serialised as part of a full inference pipeline for zero-setup deployment.
- Deployed the trained model as a live Streamlit app enabling real-time fake/real classification with per-class confidence scores, requiring no retraining for end users.
